...
export class BlockchainClient {
  ...
  public async registerAndEnrollUser(enrollId: string, attributes?: any[]): Promise<Member> {
    ...
    return new Promise<Member>((resolve: (member: Member) => void, reject: (error: Error) => void) => {
      this.chain.getUser(enrollId, (err: any, userObject: Member) => {
        ...
        this.chain.registerAndEnroll(registrationRequest, (err: Error, member?: Member) => {
          if (err) {
